...
export class P3DocumentationManager {
	...
	public load(): Promise<void> {
        return new Promise(async (resolve, _reject) => {
			...
			let buffer = await fs.readFileSync(filePathInstructions, 'utf8');
            let lines = (await buffer.toString()).split(/\r\n|\r|\n/g);
            let lineIndex = 0;
            for (let line of lines) {
                if (line.length > 0) {
					try {
						...
						console.log("Adding instructions docs: " + inst.name);
					} catch(err){
			...
			buffer = await fs.readFileSync(filePathRegisters, 'utf8');
            lines = (await buffer.toString()).split(/\r\n|\r|\n/g);
            lineIndex = 0;
            for (let line of lines) {
                if (line.length > 0) {
					try {
